...
def detect_membranes(seg):
    ...
    for z in tqdm(range(Z)):
        contours, hierarchy = cv.findContours(seg[z, :, :], cv.RETR_FLOODFILL, cv.CHAIN_APPROX_NONE)
...
def preprocess_flyem(dir_out, bounds):
    ...
    em = CloudVolume("gs://neuroglancer-janelia-flyem-hemibrain/emdata/raw/jpeg", use_https=True, mip=0)
    seg = CloudVolume("gs://neuroglancer-janelia-flyem-hemibrain/v1.2/segmentation", use_https=True, mip=0)

    em_data = np.squeeze(em[bounds[0]:bounds[1]:5, bounds[2]:bounds[3], bounds[4]:bounds[5]])
    seg_data = np.squeeze(seg[bounds[0]:bounds[1]:5, bounds[2]:bounds[3], bounds[4]:bounds[5]])
