...
import fastremap
def main(point=None, vol_size=(1500,1500,150), scale=(2,2,1), output_dir='training_data'):
    ...
    em = em_vol.download_point(point, size=vol_size,mip=0)
    
    with h5py.File(os.path.join(output_dir, "em_volume"),'w') as f:
        f.create_dataset("raw",data=np.array(em))
    seg = seg_vol.download_point(point, size=vol_size,mip=0)
    seg = np.array(seg)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--vol_size', help="Size of downloaded volume")
