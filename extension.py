...
class Extension():
    ...
    def get_bounds(self, endp):
        ...
        self.bound_EM = get_bounds(
            self.endpoint, self.radius, self.unet_bound_mult)
    ...
    def gen_membranes(self, intern_pull=True, restitch_gaps=False):
        ...
        self.mem_seg, self.seg, self.em, errors_gap, errors_linear = em_analysis(
            self.em_big, self.seg_remap, self.cnn_weights, self.device, self.bound_EM, intern_pull=intern_pull, restitch_gaps=restitch_gaps, direction=self.direction)
        self.errors = errors_gap + errors_linear
        # [int((unet_bound_mult-1)*radius[0]):int((unet_bound_mult+1)*radius[0]),
        self.mem_to_run = self.mem_seg
    ...
    def dist_transform_merge(self):
        ...
        for i in range(1, ms.shape[2]-1):
            ...
            if np.sum(ms[..., i] > 0) < 1000:
                skel_ret[..., i] = 0
            skel_ret[..., i] = scipy.ndimage.distance_transform_edt(
                ms[:, :, i])
        self.edt = skel_ret
        ...
        seg_direcs = np.argwhere(
            np.sum(np.sum(self.seg_remap == seg_rid, axis=0), axis=0) > 0)
        ...
        if len(res_list) > 0:
            ...
            for k, v in results_dict.items():
                if v > .05*max_val:
                    ext_direcs = np.argwhere(
                        np.sum(np.sum(self.seg_remap == k[1], axis=0), axis=0) > 0)
                #         print(k, ext_direcs)
                    if self.direction == 1:
    ...
    def run_agents(self, nsteps=200, n_agents=100):
        ...
        self.agent_queue = self.create_queue(
            n_agents, sampling_type="extension_ep")
    ...
    def create_queue(self, n_pts, sampling_type="extension_only"):
        ...
        if sampling_type == "extension":
            q = []
            ids, all_ids, centers, sizes_list = self.get_contacts(
                self.seg, self.root_id, conv_size=6)
    ...
    def get_seg_slices(self):
        ...
        seg_mask = np.in1d(seg_mask, self.seg_root_id).reshape(
            seg_mask.shape).astype(np.uint8)
        sizes_list = []
        centers_list = []
    ...
    def agent_merges(self, pos_histories):
        ...
        self.pos_matrix, self.merge_d = scripts.create_post_matrix(
            pos_histories, self.seg, self.mem_to_run.shape, merge=True)
        print("Merge_d time", time.time() - tic)
        self.merges = scripts.position_merge(self.endpoint, self.root_id, self.merge_d, self.endpoint *
                                             self.resolution, self.n_errors, self.mean_errors, self.max_error, self.seg_remap_dict)
    ...
    def get_data(self, seg_or_sv='sv'):
        try:
            vol = CloudVolume(
                "s3://bossdb-open-data/iarpa_microns/minnie/minnie65/em", use_https=True, mip=0)
...
def get_endpoint(ep_param, delete, endp, root_id, nucleus_id, time_point):
    if ep_param == 'sqs':
        ...
        nucleus_id = int(
            ep_msg.message_attributes['nucleus_id']['StringValue'])
...
def em_analysis(em, seg, cnn_weights, device, bound_EM, intern_pull=True, restitch_gaps=True, direction=0, zero_threshold=.5):
    ...
    if intern_pull == False:
        if cnn_weights == 'thick':
            mem_seg = membranes.segment_membranes(
                em, pth="./membrane_detection/best_metric_model_segmentation2d_dict.pth", device_s=device)
        elif cnn_weights == 'thin':
            mem_seg = membranes.segment_membranes(
                em, pth="./membrane_detection/model_CREMI_2D.pth", device_s=device)
        # import pickle
    ...
    if restitch_gaps:
        ...
        errors_zero_template = scripts.detect_artifacts_template(
            em, errors_gap)
    ...
    while True:
        last_frame = em[:, :, start]
        print(1, start, np.sum(last_frame == 0) /
              np.product(mem_seg.shape[:2]))
...
def save_merges(save, merges, root_id, nucleus_id, time_point, endpoint, weights_dict, bound,
    ...
    if save == "nvq":
        ...
        print("Posted", str(root_id), str(
            nucleus_id), end, weights_dict, metadata)
        if len(merges) == 0:
            weights_dict = {}
        C.post_agent(str(root_id), str(nucleus_id), end,
                     weights_dict, metadata, namespace)
...
def visualize_merges(merges_root, seg, root_id):
    ...
    for i, seg_id in enumerate(mset):
        ...
        opacity_merges[seg == seg_id] = merges_root[(merges_root.M1 == seg_id) | (
            merges_root.M2 == seg_id)]["Weight"] / sum_weight
