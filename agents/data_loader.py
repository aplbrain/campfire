...
def get_seg(x_pre, x_post, y_pre, y_post, z_pre, z_post):
    seg = CloudVolume(
        "s3://bossdb-open-data/iarpa_microns/minnie/minnie65/seg", use_https=True, mip=0
    )
...
def get_nuclei(x_pre, x_post, y_pre, y_post, z_pre, z_post):
    seg = CloudVolume(
        "s3://bossdb-open-data/iarpa_microns/minnie/minnie65/nuclei",
        use_https=True,
        mip=0,
    )
    return seg[x_pre:x_post, y_pre:y_post, z_pre:z_post]
...
def supervoxels(
    ...
    z_pre,
    z_post,
    simplify_supervoxels=True,
    as_unique_list=False,
):
    cave_client = CAVEclient("minnie65_phase3_v1")
    sv_ids = CloudVolume(
        "s3://bossdb-open-data/iarpa_microns/minnie/minnie65/ws",
        use_https=True,
        mip=0,
        bounded=False,
        autocrop=True,
    ...
    if simplify_supervoxels:
        ...
        sv_ids = np.array([seg_dict[x] for x in unique_sv_id])[inv].reshape(
            sv_ids.shape
        )

    if as_unique_list:
        sv_ids = np.reshape(
            sv_ids, (1, sv_ids.shape[0] * sv_ids.shape[1] * sv_ids.shape[2])
        )
...
def get_num_soma(root_id: str):
    ...
    soma = cave_client.materialize.query_table(
        "nucleus_neuron_svm", filter_equal_dict={"pt_root_id": root_id}
    )
    return len(soma)
...
if __name__ == "__main__":
    ...
    scdf = get_syn_counts(864691136025170522)
