...
def create_queue(
    ...
    if sampling_type == "extension":
        q = []
        ids, all_ids, centers, sizes_list = get_contacts(
            segmentation, root_id, conv_size=6
        )
    ...
    if sampling_type == "extension_only":
        ...
        polarity = (
            polarity_table.dendrite_df[polarity_table.dendrite_df.seg_id == root_id]
            .iloc[0]
            .polarity
        )
        data_size = data_shape[0] * data_shape[1]
...
def get_contacts(seg, root_id, conv_size=3):
    ...
    for rid in close_ids:
        ...
        for z in range(np.min(points_z), np.max(points_z) + 1):
            ...
            if (
                points_slice.shape[0] > 0
                and seg[int(mean_pt[0]), int(mean_pt[1]), z] == rid
            ):
...
def multi_slice_viewer(membranes, positions=None, start_slice=-1):
    ...
    ax.imshow(
        membranes[:, :, ax.index].T,
        interpolation="nearest",
        cmap=mem_colors,
        origin="upper",
    )
    ax.imshow(
        positions[:, :, ax.index].T,
        interpolation="nearest",
        cmap=agent_colors,
        origin="upper",
    )
...
class is_dendrite:
    ...
    def __init__(self, endpoint, seg_ids):
        ...
        post_ids = client.materialize.query_table(
            synapse_table,
        ...
        pre_ids = client.materialize.query_table(
            synapse_table,
    ...
    def dendrite_filt(self, root_id, seg_id):
        polarity_root = self.dendrite_df[
            self.dendrite_df.seg_id == root_id
        ].polarity.iloc[0]
...
def shift_detect(em, radius, n_windows, n_hits=-1, threshold=10, zero_threshold=0.75):
    if n_hits == -1:
        if n_windows > 1:
            n_hits = n_windows - 1
        else:
        ...
        for k in range(n_windows):
            ...
            mse = np.sqrt(
                np.sum(
                    np.square(
                        np.array(min_loc)
                        - np.array((centers[k] - radius, centers[k] - radius))
                    )
                )
            )
            means.append(mse)
...
class Intersection:
    ...
    def sortList(self):
        ...
        if type(self.sets[0]) == tuple:
            sets_unpack = [
                (*coord, id_num) for coord, id_num in zip(self.sets, self.id_list)
            ]
        else:  # intended for np.ndarray
            sets_unpack = [
                (*coord.astype(int), id_num)
                for coord, id_num in zip(self.sets, self.id_list)
            ]
    ...
    def clash(self):
        return [
            [self.sets[i][-1], self.sets[i + 1][-1], *self.sets[i][:-1]]
            for i in range(0, len(self.sets) - 1)
            if (
                np.array_equal(self.sets[i][:-1], self.sets[i + 1][:-1])
                and not self.sets[i][-1] == self.sets[i + 1][-1]
            )
        ]
    ...
    def merge(self, clash, soma, polarity, ep=-1, root_id=-1):
        ...
        for c in weight_dict:
            ...
            merge_df = merge_df.append(
                {
                    "EP": ep,
                    "root_id": root_id_compare,
                    "seg_id": to_merge,
                    "Weight": weight,
                    "Merge Locations": locs,
                },
                ignore_index=True,
            )
        return merge_df
