...
def endpoints(queue_url_rid, namespace='Errors_GT', save='nvq', delete=False):
    ...
    if save == 'sqs':
        ...
        if high_confidence_tips.shape[0] == 0 and med_confidence_tips.shape[0] == 0 and low_confidence_tips.shape[0]:
            s1 = nvc_post_point(C, np.array([0, 0, 0]), "dxenes1", namespace, "no_tips_found", time_point, metadata)
        if high_confidence_tips.shape[0] > 0:
            s1 = nvc_post_point(C, (high_confidence_tips).astype(int), "dxenes1", namespace, "error_high_confidence_", time_point, metadata)
        else:
            s1 = -1
        if med_confidence_tips.shape[0] > 0:
            s2 = nvc_post_point(C, (med_confidence_tips).astype(int), "dxenes1", namespace, "error_med_confidence", time_point, metadata)
        else:
            s2 = -1
        if low_confidence_tips.shape[0] > 0:
            s3 = nvc_post_point(C, (low_confidence_tips).astype(int), "dxenes1", namespace, "error_low_confidence", time_point, metadata)
        else:
            s3 = -1
...
def errors_defects_facets(queue_url_rid, namespace='Errors_defects', save='nvq', delete=False):
    ...
    if save == 'nvq':
        ...
        if sorted_encapsulated_send.shape[0] > 0:
            s1 = nvc_post_point(C, (sorted_encapsulated_send).astype(int), "dxenes1", namespace, "encapsulated_points", time_point, metadata, [])
        else:
            s1 = -1
        if facets_send_final.shape[0] > 0:
            s2 = nvc_post_point(C, (facets_send_final).astype(int), "dxenes1", namespace, "flats", time_point, metadata, rf)
        else:
            s2 = -1
        if errors_send.shape[0] > 0:
            s3 = nvc_post_point(C, (errors_send).astype(int), "dxenes1", namespace, "general_error_locs", time_point, metadata, re)
        else:
            s3 = -1
        if errors_tips_send.shape[0] > 0:
            s4 = nvc_post_point(C, (errors_tips_send).astype(int), "dxenes1", namespace, "tip_error_locs", time_point, metadata, re_ep)
        else:
            s4 = -1
        #print("Posted", s1, s2, s3)
    if delete:
...
def run_endpoints(end, namespace="tips", save='nvq', delete=False):
    ...
    queue_url_rid = sqs.get_or_create_queue("campfire-queue")
