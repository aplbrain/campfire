...
from agents.scripts import (
    precompute_membrane_vectors,
    create_post_matrix,
    merge_paths,
    get_soma,
)  # get_syn_counts
...
def drive(
    ...
    mem_seg = scripts.remove_shifts(
        mem_seg, errors_shift + errors_zero, zero_or_remove="zero"
    )

    mem_to_run = mem_seg[
        int((unet_bound_mult - 1) * radius[0]) : int((unet_bound_mult + 1) * radius[0]),
        int((unet_bound_mult - 1) * radius[1]) : int((unet_bound_mult + 1) * radius[1]),
        :,
    ].astype(float)
    print(f"Seg time: {time.time() - tic}")
    tic = time.time()

    compute_vectors = scripts.precompute_membrane_vectors(
        mem_to_run, mem_to_run, precomp_file_path, 3
    )
    print(f"Convolution time: {time.time() - tic}")

    sensor_list = [
        (agents.sensor.BrownianMotionSensor(), 0.1),
        (agents.sensor.PrecomputedSensor(), 0.2),
        (agents.sensor.MembraneSensor(mem_to_run), 0),
    ]

    pos_histories, seg_ids, agent_ids = run_agents(
        mem=mem_to_run,
    ...
    pos_matrix, merge_d = scripts.create_post_matrix(
        pos_histories, seg, mem_to_run.shape, merge=True
    )
...
def visualize_merges(merges_root, seg, root_id):
    ...
    for i, seg_id in enumerate(mset):
        ...
        opacity_merges[seg == seg_id] = (
            merges_root[(merges_root.M1 == seg_id) | (merges_root.M2 == seg_id)][
                "Weight"
            ]
            / sum_weight
        )
...
def drive_old(
    ...
    for _ in range(n):
        ...
        vol = CloudVolume(
            "s3://bossdb-open-data/iarpa_microns/minnie/minnie65/em",
            use_https=True,
            mip=0,
        )
        try:
        ...
        mem_to_run = mem_seg[
            int((unet_bound_mult - 1) * radius[0]) : int(
                (unet_bound_mult + 1) * radius[0]
            ),
            int((unet_bound_mult - 1) * radius[1]) : int(
                (unet_bound_mult + 1) * radius[1]
            ),
            :,
        ].astype(float)
        print(f"Seg time: {time.time() - tic}")
        ...
        compute_vectors = precompute_membrane_vectors(
            mem_to_run, mem_to_run, precomp_file_path, 3
        )
        print(f"Convolution time: {time.time() - tic}")
        ...
        pos_histories, seg_ids, agent_ids, soma, polarities = run_agents(
            mem=mem_to_run,
            sensor_list=sensor_list,
            precompute_fn=precomp_file_path + ".npz",
            max_vel=0.9,
        ...
        # find merges of agents from root_id
        pos_matrix = create_post_matrix(
            pos_histories, mem_to_run.shape, soma, polarities
        )
        ...
        if save == "nvq":
            ...
            pickle.dump(
                mem_seg,
                open(f"./data/INTERNmem_{duration}_{root_id}_{endpoint}.p", "wb"),
            )

            pickle.dump(
                bound_EM,
                open(f"./data/INTERNBOUNDS_{duration}_{root_id}_{endpoint}.p", "wb"),
            )
            vol[
                bound_EM[0] : bound_EM[1],
                bound_EM[2] : bound_EM[3],
                bound_EM[4] : bound_EM[5],
            ] = mem_seg.astype(np.uint64)
