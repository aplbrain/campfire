...
def membrane_seg(radius=(200,200,30), resolution=(2,2,1), unet_bound_mult=1.5, save='pd',device='cpu',
    ...
    for i in range(gt_df.shape[0]):
        ...
        'cuda', save, nucleus_id, time_point, endpoint)
        ext.get_bounds(endpoint)
        success = ext.membrane_seg_save()
        print(i, gt_df.iloc[i].Root_id, endpoint, time.time()-tic, success)
