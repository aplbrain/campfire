...
def endpoints(queue_url_rid, namespace='Errors_GT', save='nvq', delete=False):
    ...
    print("RID", root_id)
    good_tips_thick, good_tips_thin, good_tips_bad_thick, good_tips_bad_thin, all_tips, all_flat = endpoints_from_rid(
        root_id)
    if delete:
        root_id_msg.delete()
    if save == 'sqs':
        ...
        if good_tips_thick.shape[0] > 0:
            s1 = nvc_post_point(C, (good_tips_thick).astype(
                int), "Justin", namespace, "error_high_confidence_thick", time_point, metadata)
        else:
            s1 = -1
        if good_tips_thin.shape[0] > 0:
            s2 = nvc_post_point(C, (good_tips_thin).astype(
                int), "Justin", namespace, "error_high_confidence_thin", time_point, metadata)
        else:
            s2 = -1
        if good_tips_bad_thick.shape[0] > 0:
            s3 = nvc_post_point(C, (good_tips_bad_thick).astype(
                int), "Justin", namespace, "error_low_confidence_thick", time_point, metadata)
        else:
            s3 = -1
        if good_tips_bad_thin.shape[0] > 0:
            s4 = nvc_post_point(C, (good_tips_bad_thin).astype(
                int), "Justin", namespace, "error_low_confidence_thin", time_point, metadata)
        else:
            s4 = -1
        if all_tips.shape[0] > 0:
            s5 = nvc_post_point(C, (all_tips).astype(
                int), "Justin", namespace, "all_tips", time_point, metadata)
        else:
            s5 = -1
        all_flat = np.array(all_flat)
        if all_flat.shape[0] > 0:
            s6 = nvc_post_point(C, (all_flat).astype(
                int), "Justin", namespace, "all_flats", time_point, metadata)
        else:
            s6 = -1
...
def nvc_post_point(C, points, author, namespace, name, status, metadata):
    if len(points.shape) == 1:
        C.post_point([str(p) for p in points], author,
                     namespace, name, status, metadata)
    elif len(points.shape) == 0:
        return 0
    else:
        for pt in points:
            C.post_point([str(p) for p in pt], author,
                         namespace, name, status, metadata)
    return 1
...
# point_id is just a unique index
...
def segment_points(root_id, endpoint, point_id, radius=(200, 200, 30), resolution=(8, 8, 40), unet_bound_mult=1.5, save='pd', device='cpu',
    ...
    # ext.merges stores a dict w seg_ids and confidence that they are the extensions
    return ext, 1
...
def run_nvc_agents(namespace, namespace_agt, radius=(300, 300, 30), rez=(8, 8, 40), unet_bound_mult=1, save='nvq', device='cpu', direction_test=True):
    print(namespace, namespace_agt, radius, rez,
          unet_bound_mult, save, device, direction_test)
    points = get_points_nvc({"namespace": namespace, 'type': [
                            'error_high_confidence_thick', 'error_high_confidence_thin'], 'agents_status': 'open'})
    idx = points.index
    n_machines = 2
    ...
    while True:
        for p in range(points.shape[0]):
            ...
            ext, s = segment_points(rid, pt, idx[p], radius=radius, resolution=rez, unet_bound_mult=unet_bound_mult,
                                    save=save, device=device, namespace=namespace_save, direction_test=direction_test)
            if s < 0:
