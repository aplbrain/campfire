...
def send_one(queue_name, message_body, message_attributes, **kwargs):
    ...
    while retries > 0:
        response = sqs.send_message(
            QueueUrl=queue_url, MessageBody=message_body, Attributes=message_attributes
        )
        if len(response.get("Failed", [])) == 0:
            break
        retries -= 1
...
def construct_endpoint_entries(endpoint_batch: Iterable[List], root_id: str) -> Dict:
    """
...
def send_mem_to_cloud(mem, bound):
    ...
    membranes = CloudVolume(
        "s3://neuvue-data/minnie65-membranes", progress=False, non_aligned_writes=True
    )
    print(
        bound, bound[1] - bound[0], bound[3] - bound[2], bound[5] - bound[4], mem.shape
    )
